...
public class Session {
    ...
    private Session(String email) {
        long configExpr = IDMService.getConfigs().getExpiration();
        sessionID = Token.generateToken();
        this.email = email;
        this.timeCreated = new Timestamp(System.currentTimeMillis());
        this.lastUsed = timeCreated;
        this.exprTime = new Timestamp(System.currentTimeMillis() + configExpr);
        ServiceLogger.LOGGER.info("= = = = = = = = Expired length: " + configExpr);
        //changed here. Before: this.exprTime = new Timestamp(System.currentTimeMillis() + );
    }
    ...
    /*
        ...
        than  time after the session was initially created.
     */
    public boolean isDataValid() {
        ...
        long configTimeOut = IDMService.getConfigs().getTimeout();
        Timestamp timeoutTime = new Timestamp(lastUsed.getTime() + configTimeOut);
        ServiceLogger.LOGGER.info(" = = = = = = = = Timeout length: " + configTimeOut);
        //Changed here. Before: Timestamp timeoutTime = new Timestamp(lastUsed.getTime() + );
